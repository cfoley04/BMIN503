...
table(nhanes$DMDEDUC2, useNA = "ifany") #Includes missing outcome
table(nhanes$RIAGENDR, nhanes$DMDEDUC2) #Bivariate tables
```
...
library(dplyr)
nhanes <- dplyr::rename(nhanes, id = SEQN, gender = RIAGENDR, age = RIDAGEYR, race = RIDRETH1, education = DMDEDUC2, income = INDHHIN2, health.provider = HUQ040)
head(nhanes)
```
...
nhanes <- select(nhanes, c(id:education, adult.status))
```
...
#### Pipe operator
A useful way to perform several operations with `dplyr` is to use the pipe operator `%>%`, which will chain commands without saving several intermediate tables and without having to keep explicitly referring to dataframes.
```{r eval = TRUE}
nhanes %>% 
...
### Editing Text
When changing variable names or levels/entries, we often have to change text, rather than numerical values. A nice package to use if you are working with strings is [`stringr`](https://stringr.tidyverse.org/). Some base functions that are helpful to edit text:
...
gsub("[.]", "", x) #The "." needs to be escaped by brackets
