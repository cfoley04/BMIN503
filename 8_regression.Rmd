...
nhanes <- rename(nhanes, id = SEQN, sex = RIAGENDR, age = RIDAGEYR, race = RIDRETH1, education = DMDEDUC2, income = INDHHIN2, health.provider = HUQ040, wheezing = RDQ070, asthma = MCQ010, voc = WTSVOC2Y, fvc = SPXNFVC, fev1 = SPXNFEV1) %>%
...
If x were a binary variable, like sex, we could simulate it using `rbinom`, which corresponds to the binomial distribution.
```{r, eval = TRUE}
...
What about other demographic variables? After looking at plots, which of the linear models seem most appropriate, and which ones might need modification?
```{r eval = TRUE}
ggplot(nhanes, aes(x = age, y = fev1)) +
    geom_point(color = "blue")
summary((lm(fev1 ~ age, data = nhanes)))

ggplot(nhanes, aes(x = sex, y = fev1)) +
    geom_boxplot()
summary((lm(fev1 ~ sex, data = nhanes)))
...
```{r eval = TRUE}
summary.lm(lm(fev1 ~ fvc + sex, data = nhanes))
summary.lm(lm(fev1 ~ fvc + sex + fvc:sex, data = nhanes))
...
```{r eval = TRUE}
ggplot(data = nhanes, aes(x = factor(asthma), fill = sex)) +
    geom_bar(position = "fill")

chisq.test(table(nhanes$asthma, nhanes$sex)) 
summary((glm(asthma ~ sex, data = nhanes, family = binomial())))
exp(coef(glm(asthma ~ sex, data = nhanes, family = binomial()))) #Odds ratios
...
```{r eval = TRUE}
asthma.fit <- glm(asthma ~ fev1 + age + sex + race, data = nhanes, family = binomial())
summary(asthma.fit)
